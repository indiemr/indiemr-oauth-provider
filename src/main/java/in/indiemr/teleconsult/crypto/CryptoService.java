package in.indiemr.teleconsult.crypto;
...
public class CryptoService {
    ...
    public CryptoService(TeleconsultProperties props) {
        this.props = props;
    }

    private byte[] key() throws Exception {
        String secret = props.getEncKey();
        if (secret == null || secret.isBlank()) {
            throw new IllegalStateException("TELECONSULT_ENC_KEY is not set");
        }
    ...
    public String decrypt(String payload) throws Exception {
        ...
        if (parts.length != 3) throw new IllegalArgumentException("Malformed encrypted payload");
    ...
    private String hmac(String body) throws Exception {
        ...
        mac.init(new SecretKeySpec(props.getEncKey().getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
